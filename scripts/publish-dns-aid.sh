...
#
# Only the _index entrypoint is published: this site does not currently expose
# MCP or A2A agent endpoints, so _mcp / _a2a records are intentionally omitted.
#
# Optional (when ENABLE_DNSSEC=1): enable DNSSEC on the zone via API. If the
# domain registrar is not Cloudflare Registrar, copy the returned DS record to
# the registrar so the chain of trust is complete.
#
# Usage:
